...
from model.Encoder import Lung_clinical_dfs
...
def main():
    ...
    for k in dfs:
        dfs[k] = dfs[k][dfs[k]["Reccur.months"] > 0]

    drop_cols = ["Survival.status", "Survival.months"]
    ...
    encoder = Lung_clinical_dfs(in_dim=in_dim).to(device)
    gae = VGAE(encoder).to(device)

    optimizer = optim.Adam(gae.parameters(), lr=0.0001571745559666506, weight_decay=0.00037126694366927286)
    ...
    cph = CoxPHFitter(penalizer=0.0007437832844579877)
