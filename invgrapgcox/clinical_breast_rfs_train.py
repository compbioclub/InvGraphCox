...
from model.Encoder import Breast_clinical_rfs
...
def main():
    ...
    drop_cols = ["Survival.months", "Survival.status", "Cohort"]
    ...
    encoder = Breast_clinical_rfs(in_dim=in_dim).to(device)
    gae = VGAE(encoder).to(device)

    optimizer = optim.Adam(gae.parameters(), lr=6.268957841641548e-05, weight_decay=1.848337639035723e-09)
    ...
    cph = CoxPHFitter(penalizer=0.00038969847495602395)
